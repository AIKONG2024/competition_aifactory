...
def conv2d_block(input_tensor, n_filters, kernel_size=3, batchnorm=True):
    """주어진 매개변수로 2개의 컨볼루션 레이어를 추가하는 함수"""
    # 첫 번째 레이어
...
def pyramid_feature_fusion(input_feature, pyramid_levels, n_filters, batchnorm=True):
    """다양한 피라미드 레벨에서 특징들을 융합하는 함수"""
    fused_features = input_feature
    for level in pyramid_levels:
        # 컨볼루션 적용 후 input_feature의 크기와 일치하도록 업샘플링
        level_feature = Conv2D(filters=n_filters, kernel_size=(3, 3), padding='same')(level)
        upsample_size = (input_feature.shape[1] // level_feature.shape[1], input_feature.shape[2] // level_feature.shape[2])
        level_feature = UpSampling2D(size=upsample_size)(level_feature)
        
        if batchnorm:
            level_feature = BatchNormalization()(level_feature)
        
        # 업샘플링된 피처를 fused_features와 결합
        fused_features = concatenate([fused_features, level_feature])
        
    # 융합된 특징들을 부드럽게 혼합하기 위해 conv2d_block 적용
    fused_features = conv2d_block(fused_features, n_filters, kernel_size=3, batchnorm=batchnorm)
    return fused_features

def get_pretrained_pyramid_unet(input_height=256, input_width=256, nClasses=1, n_filters=16, dropout=0.5, batchnorm=True, n_channels=3):
    # VGG16 모델 로드, 최상위 분류 계층 제외
    base_model = VGG16(weights='imagenet', include_top=False, input_shape=(input_height, input_width, n_channels))
    
    # 입력 정의
    inputs = base_model.input
    
    # 스킵 연결을 위해 VGG16 모델의 특정 레이어 사용
    s1 = base_model.get_layer("block1_conv2").output
    s2 = base_model.get_layer("block2_conv2").output
    s3 = base_model.get_layer("block3_conv3").output
    s4 = base_model.get_layer("block4_conv3").output
    ...
    # 디코더에서 Pyramid Feature Fusion
    pyramid_levels = [s1, s2, s3, s4]
    fused_features = pyramid_feature_fusion(bridge, pyramid_levels, n_filters*8, batchnorm=batchnorm)
    
    # 특징을 융합한 후 디코더 경로를 따름
    d1 = UpSampling2D((2, 2))(fused_features)
    d1 = conv2d_block(d1, n_filters*8, kernel_size=3, batchnorm=batchnorm)
